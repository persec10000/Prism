...
import {AppAlert, GoLogin, ellipsis} from '../../Helpers';
...
export default class SelectedSubFolder extends React.Component {
    ...
    async GetAllVDR() {
        ...
        var params = this.props.navigation.state.params
        await fetch(api.GetAllVdrMetaTemplateFoldersAndFiles_URL + "/" + this.state.RoleID + "/" + this.state.ID + "/" + params.RepositoryID + "/" + params.MetaTemplateID + "/" + this.state.folderID)
            .then(res => res.json())
            .then(res => {
                if (res.IsSuccess == "1") {
                    if (res.Message != "[]") {
    ...
    selectAction(item){
        if(this.state.action =="1"){
            if(item.FolderID != currentFolderID){
                AppAlert({
                    title: `Copy to ${item.FolderName}`,
                    subtitle: 'Sure Copy file to selected folder?',
                    ok: () => {
                        this.copyItem(item);
                    },
                    cancel: () => {
            ...
            if(this.state.action =="2") {
                if(item.FolderID != currentFolderID){
                AppAlert({
                    title: `Move to ${item.FolderName}`,
                    subtitle: 'Sure Move file to selected folder?',
                    ok: () => {
                        this.moveItem(item);
                    },
                    cancel: () => {
    ...
    alert(item){
        ...
        //     Alert.alert(
        //         item.FolderName,
        //             'Are you sure this folder you to select ?',
        //         [
        //             {text: 'Cancel', style: 'cancel'},
        //             {text: 'OK', onPress: () => this.selectAction(item)},
        //         ],
        //         { cancelable: false }
        //     )
        // }
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) =>
                           ...
                           <View style={styles.uperCard}>
                                <TouchableOpacity
                                    onPress={() => this.openNext(item)}
                                    onLongPress={()=> this.selectAction(item)}
                                >
