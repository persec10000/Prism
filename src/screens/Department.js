...
export default class Departments extends React.Component {
    ...
    async GetAllVDR() {
        ...
        await fetch(api.GetAllDepartments_URL + "/" + this.state.RoleID + "/" + this.state.ID + "/" + params.RepositoryID).then(res => res.json())
            .then(res => {
                if (res.IsSuccess == "1") {
    ...
    search(text) {
        ...
        for (i; i < dataSource.length; i++) {
            if (dataSource[i].DepartmentName.toLowerCase().indexOf(text.toLowerCase()) >= 0) {
                        ...
                        <FlatList
                            ...
                            renderItem={({ item }) =>
                                ...
                                <View style={styles.uperCard}>
                                    <TouchableOpacity onPress={() => this.props.navigation.navigate("SelectedFolder", {
                                        CreatedBy: item.CreatedBy, ID: item.ID, RepositoryID: item.RepositoryID, DeptID: item.DeptID,
                                        DepartmentName: item.DepartmentName, MetaTemplateName: item.MetaTemplateName, MetaTemplateID: item.MetaTemplateID,
                                        ...
                                        <View style={styles.uperCardChild}>
                                            ...
                                            <View>
                                                <Text style={styles.itemText}>{ellipsis(item.DepartmentName)}</Text>
                                            </View>
                                        </View>
                                    </TouchableOpacity>
